...
- **Bare keys use xdotool on sessions:** a `press` action with no `selector` (e.g. Enter, Tab, Escape) is sent as a real X11 key event via `xdotool --display <xephyr> key <keysym>` to the session's Xephyr display (`virtdisplay.ts` → `pressKeyOnDisplay`), so it behaves like physical keyboard input to the browser window rather than a CDP injection that only reaches the page's focused element. Playwright key names are mapped to X keysyms (`toXdotoolKeysym`; letters/digits/F-keys pass through). One-shot tools have no Xephyr, so their bare keys fall back to `page.keyboard.press`.
